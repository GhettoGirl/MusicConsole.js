...
toKanaWithDakutenIncluded: function(string)
{
    ...
    for (let i = 0; i < buf.length; i++)
    {
        ...
        if (buf[i] == this.m_dakuten.COMBINING_VOICED_SOUND_MARK)
        {
            ...
            {
                buf = global.jsext.String.splice(buf, i - 1, 2,
                          this.m_kanadakutenmap[this.m_kanamap.indexOf(buf[i - 1])]);
                i--;
            }
        }
        ...
        if (buf[i] == this.m_dakuten.COMBINING_SEMI_VOICED_SOUND_MARK)
        {
            ...
            {
                buf = global.jsext.String.splice(buf, i - 1, 2,
                          this.m_kanadakutenmap[this.m_kanamap.indexOf(buf[i - 1]) + 10]);
                i--;
            }
        }
    }
...
toKanaWithSeparateDakuten: function(string)
{
    ...
    for (let i = 0; i < buf.length; i++)
    {
        ...
        if (index != -1)
        {
            ...
            if (index <= 40)
            {
                buf = global.jsext.String.splice(buf, i, 1,
                          this.m_kanamap[index] + this.m_dakuten.COMBINING_VOICED_SOUND_MARK);
                i++;
            }
            ...
            {
                buf = global.jsext.String.splice(buf, i, 1,
                          this.m_kanamap[index - 10] + this.m_dakuten.COMBINING_SEMI_VOICED_SOUND_MARK);
                i++;
            }
        }
    }
