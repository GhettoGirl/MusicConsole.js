...
// returns the player for the given media type or undefined
...
method.execute = function(mediafile, mediatype)
{
    ...
    // Player Enforcement
    if (typeof mediatype == "number" && mediafile != MediaType.None)
    {
        player = this.playerForMediatype(mediatype);
        if (typeof player == "undefined")
        {
            //console.error("MediaPlayerController: no player found for media type [" + named_mediatype(mediatype) + "]");
            return -1;
        }
    }
    ...
    else
    {
        ...
        if (typeof player == "undefined")
        {
            //console.error("MediaPlayerController: no player found for filetype [" + mediafile.m_extension + "]");
            return -1;
        }
    }

    // Replace %f in argument list or append location if not existent
    var arguments = player.arguments;
    var f_index = arguments.indexOf('%f');
    if (f_index != -1)
    {
        arguments[f_index] = mediafile.m_path;
    }

    else
    {
        arguments.push(mediafile.m_path);
    }

    mediafile.print();
    var results = exec(player.command, arguments, {stdio: 'inherit'});
    arguments.length = 0;
