...
const randomizer = new (require('utils/randomizer.js'));
        ...
        for (const mediafile of filterlist)
        {
            for (const searchterm of mediafile.searchTerms())
            {
                ...
                for (let i = 0; i < without_any_of_this_keycount; i++)
                {
                    ...
                    {
                        //console.log(without_any_of_this[i].pattern + " matched " + searchterm);
                ...
                {
                    if (searchterm.match(without_genre[i].pattern))
                    {
                        mark_as_dont_add = true;
                        break; // we are done here
                    }
...
method.randomMedia = function(media)
{
    if (typeof media == "number" && media != MediaType.None)
    {
        ...
        {
            return this.m_filterlist[media][randomizer.random(0, this.m_filterlist[media].length - 1)];
        }
    }

    else
    {
        return this.m_medialist[randomizer.random(0, this.m_medialist.length - 1)];
    }
}
...
method.randomMediaFiltered = function(search_term, media)
{
    ...
    if (results.length != 0)
    {
        return results[randomizer.random(0, results.length - 1)];
    }
}
