...
function SearchKeys(search_term)
{
    ...
    if (buf.indexOf('|') != -1)
    {
        tmp = buf.split('|').filter(function(value)
        {
        ...
        for (var t of tmp)
        {
            ...
            if (t.startsWith("a "))
            {
                this.m_extended_searchpatterns.push({
                    pattern: this.createSearchPattern(this.m_searchpattern.toStringRaw() + t.substr(2)),
    ...
    for (var p in this.m_extended_searchpatterns)
    {
...
method.createSearchPattern = function(search_term)
{
    ...
    search_keys = search_keys.replaceAll(' ', '.*');
    ...
    for (var i = 0; i < search_keys.length; i++)
    {
        if (search_keys[i] == '.' && search_keys[i + 1] == '*')
        {
            if (search_keys.length > i + 2)
            {
                ...
                {
                    search_keys = search_keys.splice(i, 2, '');
                    i = 0;
                }
            }
        }
...
{
    return (this.m_searchpattern.pattern.toStringRaw().replaceAll('\\.\\*', '') == '');
}
