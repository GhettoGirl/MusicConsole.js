        ...
        for (const t of tmp)
        {
            ...
            if (t.startsWith("a "))
            {
                this.m_extended_searchpatterns.push({
                    pattern: this.createSearchPattern(global.jsext.RegExp.toStringRaw(this.m_searchpattern) + t.substr(2)),
...
method.createSearchPattern = function(search_term)
{
    ...
    search_keys = global.jsext.String.replaceAll(search_keys, ' ', '.*');
    ...
    for (let i = 0; i < search_keys.length; i++)
    {
        if (search_keys[i] == '.' && search_keys[i + 1] == '*')
        {
            if (search_keys.length > i + 2)
            {
                ...
                {
                    search_keys = global.jsext.String.splice(search_keys, i, 2, '');
                    i = 0;
                }
            }
        }
...
{
    var regex_rawstr = global.jsext.RegExp.toStringRaw(this.m_searchpattern.pattern);
    var str = global.jsext.String.replaceAll(regex_rawstr, '\\.\\*', '');
    return (str == '');
}
