...
const tagreader = new (require('../TagReader')).TagReader();

const SearchTermGen = require('../searchtermgen');
...
function MediaFile(location, check, base_path, audiotypes, videotypes, moduletypes)
{
    ...
    this.m_location != '' ? this.m_location = this.m_location.splice(0, base_path.length + 1, '') : false;
...
method.init = function(prefixdeletionpatterns)
{
    // tag fields
    this.m_artist = "";
    this.m_album = "";
    ...
    for (var i of SearchTermGen.ALL)
    {
