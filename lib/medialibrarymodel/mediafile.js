...
function MediaFile(location, check, base_path, audiotypes, videotypes, moduletypes)
{
    ...
    this.m_location != '' ? this.m_location = global.jsext.String.splice(this.m_location, 0, base_path.length + 1, '') : false;
...
method.init = function(prefixdeletionpatterns, base_path)
{
    ...
    if (this.m_artist != '' || this.m_album != '' || this.m_title != '')
    {
        this.m_searchterms = this.m_searchterms.concat([this.m_artist + " " + this.m_album + " " + this.m_title]);
    }
}

// pretty print the media file on screen
//
// [extension] artist   title  album
//   colored   italic   bold   underscored
//    bold
//
// [extension] relative path in medialibrarymodel with prefixdeletionpatterns removed
//   colored   default
//   bold
//

const fmt_extension = function(str)
{
    process.stdout.write(global.ansi.style.bold
                       + global.termcolor.foreground.rgb(0, 97, 167)
                       + "[" + str + "]"
                       + global.ansi.style.reset);
};

const fmt_artist = function(str)
{
    process.stdout.write(global.ansi.style.italic
                       + str
                       + global.ansi.style.reset);
};

const fmt_album = function(str)
{
    process.stdout.write(global.ansi.style.underline
                       + str
                       + global.ansi.style.reset);
};

const fmt_title = function(str)
{
    process.stdout.write(global.ansi.style.bold
                       + str
                       + global.ansi.style.reset);
...
{
    fmt_extension(this.m_extension);
    space();

    if (this.m_title == '')
    {
        fmt_path(this.m_location.substr(0, this.m_location.length - this.m_extension.length - 1));
    }

    else
    {
        this.m_artist != '' ? (fmt_artist(this.m_artist), space()) : false;
        this.m_title  != '' ? (fmt_title(this.m_title),   space()) : false;
        this.m_album  != '' ? (fmt_album(this.m_album),   space()) : false;
    }
